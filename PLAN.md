...
- Say "send" (`SpokenSend`, `ToolResult.submitText`, `OutputEffects.pressReturn`): a dictation ending in "send" as its own sentence is pasted without it, then Return is pressed 150 ms later, only while the app that took the paste is still in front. The result carries both texts: the pipeline strips the word only when it pastes and sends; with Paste off, or when nothing took the paste, the whole text goes out. A dictation that is only "send" presses Return and logs nothing.
- Dictionary (`WordReplacements`, `voice.dictionary` as JSON): heard to written pairs, whole words, any case, longest first. Edited inline in the Voice view on a draft, handed over 0.5 s after the last change and on close.
- Tap to lock (`Tool.holdLatched`): a tap of the hold key shorter than 0.4 s keeps recording; the next tap's release finishes. Holding still works. The shell keeps its listening state while latched. Ends by itself after 10 minutes (samples are in memory) and tells the shell through `VoiceTool.onLatchEnded`. The recorder, the microphone permission and the clock are injected (`VoiceRecording`), so the lock logic is unit tested.
- Skip fillers got its chip (the option existed without a control).
- Numbers as digits (`SpokenNumbers`, Core, on by default, only while the language is English): Parakeet already writes most numbers as digits ("25%", "January 5th, 2025", "1,200", "21st"; checked with `say` recordings through `DESKPOUCH_DEMO=transcribe`), so this catches what it leaves as words: money ("$232.50"), times ("3:30 PM"), long cardinals, spoken years, decimals, percent. One to nine stay words unless a unit follows ("five dollars"). Spoken years only join when the first half is 17 to 20 ("fifty fifty" stays), 2001 to 2099 takes no comma, "at one point five of us" keeps its words. Runs after "send", before spoken punctuation.
